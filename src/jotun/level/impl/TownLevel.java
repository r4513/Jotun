		...
		for(int x = 0; x < width; x++){
			for(int y = 0; y < height; y++){
				if(x == 0 || x == width || y == 0 || y == height){
					tiles[x][y] = new WallTile(new Sprite(Sprite.wall_dark,x,y,Tile.WIDTH,Tile.HEIGHT));
				}else{
					tiles[x][y] = new GrassTile(new Sprite(Sprite.grass,x,y,Tile.WIDTH,Tile.HEIGHT));
				}
			}
		}
	}
